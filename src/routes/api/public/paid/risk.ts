...
export const Route = createFileRoute("/api/public/paid/risk")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const paid = await requirePayment(request, "/api/public/paid/risk");
        if (paid) return paid;
        ...
        const verdict = score >= 60 ? "block" : score >= 30 ? "review" : "ok";

        return new Response(
          JSON.stringify({ verdict, score, reasons, network: "arc-testnet" }),

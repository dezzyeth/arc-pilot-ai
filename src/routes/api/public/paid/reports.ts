...
export const Route = createFileRoute("/api/public/paid/reports")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const paid = await requirePayment(request, "/api/public/paid/reports");
        if (paid) return paid;
        ...
        try {
          ...
          return new Response(JSON.stringify({ ...summary, narrative: text }), {
            status: 200,
            headers: { "content-type": "application/json" },
          });
        } catch (err) {
          return new Response(JSON.stringify({ ...summary, narrative: aiErrorMessage(err) }), {
            status: 200,
            headers: { "content-type": "application/json" },
          });
        }
